...
// InfiniteMenu (React Bits) — vendored WebGL sphere menu, ported to TypeScript.
// https://reactbits.dev — JavaScript + CSS variant, adapted for this project.
import { useEffect, useRef, useState } from 'react'
import { mat4, quat, vec2, vec3 } from 'gl-matrix'
import './infinite-menu.css'

export type InfiniteMenuItem = {
  // Cover/gallery photo used as the disc background. Falls back to a neutral
  // backdrop (with the logo centered) when omitted.
  image?: string
  // Tenant logo, overlaid on top of the cover image — always contain-fit so
...
// Crops to fill the square cell, preserving the image's aspect ratio (object-fit: cover).
function drawImageCover(ctx: CanvasRenderingContext2D, img: HTMLImageElement, size: number): void {
...
// Scales down to fit within the square area without cropping (object-fit: contain).
function drawImageContain(ctx: CanvasRenderingContext2D, img: HTMLImageElement, size: number, padding = 0): void {
  const available = size - padding * 2
  const scale = Math.min(available / img.width, available / img.height)
  const w = img.width * scale
  const h = img.height * scale
  ctx.drawImage(img, padding + (available - w) / 2, padding + (available - h) / 2, w, h)
}
...
function makeBuffer(gl: WebGL2RenderingContext, sizeOrData: number | Float32Array, usage: number): WebGLBuffer {
  const buf = gl.createBuffer() as WebGLBuffer
  gl.bindBuffer(gl.ARRAY_BUFFER, buf)
  gl.bufferData(gl.ARRAY_BUFFER, sizeOrData as Float32Array, usage)
...
class InfiniteGridMenu {
  ...
  private init(onInit?: ((instance: InfiniteGridMenu) => void) | null): void {
  ...
  private initTexture(): void {
    ...
    Promise.all(
      this.items.map(async (item) => ({
        cover: item.image ? await loadImage(item.image) : null,
        logo: item.logo ? await loadImage(item.logo) : null
      }))
    ).then((loaded) => {
      if (!ctx) return
      loaded.forEach(({ cover, logo }, i) => {
        ...
        if (cover) {
          ctx.translate(x, y)
          drawImageCover(ctx, cover, cellSize)
        } else {
        ...
        if (logo) {
          if (cover) {
            // Logo badge floating over the cover photo, contain-fit so it's never stretched.
            const badgeSize = cellSize * 0.34
            const margin = cellSize * 0.06
            const bx = x + (cellSize - badgeSize) / 2
            const by = y + cellSize - badgeSize - margin

            ctx.save()
            ctx.fillStyle = 'rgba(255, 255, 255, 0.92)'
            roundedRectPath(ctx, bx, by, badgeSize, badgeSize * 0.16)
            ctx.fill()
            ctx.restore()

            ctx.save()
            ctx.translate(bx, by)
            drawImageContain(ctx, logo, badgeSize, badgeSize * 0.12)
            ctx.restore()
          } else {
            // No cover photo — center the logo on the neutral backdrop, contain-fit.
            ctx.save()
            ctx.translate(x, y)
            drawImageContain(ctx, logo, cellSize, cellSize * 0.2)
            ctx.restore()
          }
        }
      })
