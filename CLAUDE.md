...
- **Formerly open gaps, now closed**: FK constraints from `users` → `tenants.userId`/`revenueReports.userId`/`moderanSyncLog.sentBy` are now enforced (`drizzle/migrations/0002_black_umar.sql`; see `drizzle/schema.ts` for why this is safe against existing data). The old plan to hand admins a short-lived signed impersonation JWT (`lib/auth/admin-users.ts`'s `generateImpersonationToken`/`verifyImpersonationToken`) was abandoned before it was ever wired up — `app/api/admin/impersonate/route.ts` mints a normal Auth.js session JWT directly instead (never exposes a bearer token to the client), so those two functions were dead code and have been removed.
