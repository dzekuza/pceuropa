...
async function getPromo(slug: string) {
  ...
  const { data } = await supabase
    .from("promos")
    .select("title, title_en, content, content_en, image, starts_at, ends_at")
    .eq("slug", slug)
    .eq("published", true)
    .single();
  return data;
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const description = content
    ...
    .slice(0, 160);
  return {
    title: `${title} — PC Europa`,
    description:
      description || formatPromoDateRange(item.starts_at, item.ends_at, locale),
  };
}
          ...
          <div className="flex flex-col gap-6">
            <div className="flex flex-col gap-3">
              <p className="text-[#575757] text-base">
                {formatPromoDateRange(item.starts_at, item.ends_at, locale)}
              </p>
